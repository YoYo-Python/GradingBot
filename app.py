...
image = (
    ...
    .pip_install("pymupdf", "requests")
)

app = modal.App(name="paper6-grader", image=image)
...
def grade_paper_endpoint(item: dict):
    import pymupdf
    import requests

    api_key = item.get("api_key", "AQ.Ab8RN6JcLAADySNQqxyJ2KYgXiKIivvOYCPzmMYU55RWsPRD_A")
    ms_file_uri = item.get("ms_file_uri", "https://generativelanguage.googleapis.com/v1beta/files/c8opgi09eyjm")
    pdf_base64 = item.get("pdf_base64")

    if not pdf_base64:
        return {"error": "Missing pdf_base64 in payload"}
...
"""
    ...
    # 2. Upload student PDF to Gemini File API
    ...
    # 3. Annotation Placement (Matching your exact grade.py scaling and clamping logic)
    total_exam_score = 0
    ...
    for i in range(total_pages):
        ...
        score_text = f"Page Score: {score}" + (f" / {max_marks}" if max_marks else "")
        badge_w = 175.0 * scale
        badge_h = 28.0 * scale
